...
<template>
  <div id="allblog">
    <div id="allblog-table">
      ...
      <div v-if="item.blogtype==recommendList[0]" v-for="(item,index) in blogs" :key=index class="all-blog-table-item">
      ...
      <div v-if="item.blogtype==recommendList[2]" v-for="(item,index) in blogs" :key=index class="all-blog-table-item">
        <div style="width:5%;margin-left:0px">{{index+1}}</div>
        <div style="width:10%">{{item.username}}</div>
        <div style="width:15%">发布时间：{{item.publishtime | formatDate}}</div>
        <div style="width:50%" @click=showDetail(item)>{{item.blogcontent}}</div>
      ...
      <div v-if="item.blogtype==recommendList[3]" v-for="(item,index) in blogs" :key=index class="all-blog-table-item">
        <div style="width:5%;margin-left:0px">{{index+1}}</div>
        <div style="width:10%">{{item.username}}</div>
        <div style="width:15%">发布时间：{{item.publishtime | formatDate}}</div>
        <div style="width:50%" @click=showDetail(item)>{{item.blogcontent}}</div>
      ...
      <div v-if="item.blogtype==recommendList[4]" v-for="(item,index) in blogs" :key=index class="all-blog-table-item">
        <div style="width:5%;margin-left:0px">{{index+1}}</div>
        <div style="width:10%">{{item.username}}</div>
        <div style="width:15%">发布时间：{{item.publishtime | formatDate}}</div>
        <div style="width:50%" @click=showDetail(item)>{{item.blogcontent}}</div>
...
export default {
  ...
  methods: {
    ...
    getRecommendList(){
      let _this = this
       _this.$axios.get('/api/blog/getRecommendList',{
               params:{
...
<style lang='scss' src="@/assets/menu/sysrecommend.scss" scoped>
</style>